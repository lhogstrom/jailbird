'''
Assessing the significance of GEO query results using 
DMSO connections as a null distribution
...
# which of the GEO results have ratings
summRes = aFrm._id[aFrm._id.isin(rFrame.source)].values
aRes = aFrm.reindex(summRes)
...
# add geo IDs 
aNameDict = aRes.a_name.to_dict()   
matchFrm['a_name'] = matchFrm['source'].map(aNameDict)
geoDict = aRes.series.to_dict()
matchFrm['geo_id'] = matchFrm['source'].map(geoDict)
#keep only summly results which have expected connections
matchFrm['summ_txt'] = matchFrm['source'] + '_lm_summly.txt'
# matchFrm = matchFrm[matchFrm.summ_txt.isin(sFrm.columns)]
outF = wkdir + '/expected_connection_combined_table.txt'
matchFrm.to_csv(outF,sep='\t')
# which expected connections don't have directionality asigned?
needDir = matchFrm[matchFrm.direction.isnull()]
outF = wkdir + '/expected_connection_missing_directionality.txt'
needDir.to_csv(outF,sep='\t')
# summInd = [x + '_lm_summly.txt' for x in matchFrm['source']]
# summInd = list(set(summInd)) #remove dups
# sFrm = sFrm.ix[:,sFrm.columns.isin(summInd)]
...
########################################
### load DMSOs and calcualte ranks#####
########################################
...
boolFDR, valFDR = fdr(pvals=expectedFrm['perc_rank_within_pert_type'].values, alpha=.1) #,alpha=.05
expectedFrm['pass_FDR_with_percent_rank'] = boolFDR
boolFDR, valFDR = fdr(pvals=expectedFrm['p_val_dmso'].values, alpha=.1) #,alpha=.05
expectedFrm['pass_FDR_with_dmso'] = boolFDR
...
## plot p-values from dmso analysis
plt.hist(expectedFrm['p_val_dmso'],30)
plt.ylabel('freq',fontweight='bold')
plt.xlabel('p-value of expected connections based on DMSO',fontweight='bold')
plt.title('All expected GEO connections')
outF = path.join(wkdir, 'p_val_dmso_expected_connections.png')
