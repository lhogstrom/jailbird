...
def template_heatmap(args,work_dir):
	...
	for i,unique_pert in enumerate(unique_perts):
		...
		pert_scores = gcto.matrix[0,cid_inds]
		pert_doses = [doses[x] for x in cid_inds]
		tmp_tup = zip(pert_doses,pert_scores,cid_inds)
		tmp_tup.sort()
		pert_doses,pert_scores,cid_inds = zip(*tmp_tup)
		...
		if len(pert_doses) > 1:
			...
			linear_probe_corrs = cc[0:num_probes,num_probes + 1]
			log_probe_corrs = cc[0:num_probes,num_probes + 1]
			half_log_probe_corrs = cc[0:num_probes,num_probes + 1]
			quarter_log_probe_corrs = cc[0:num_probes,num_probes + 1]

			linear_probe_corrs_sort_ind = numpy.argsort(linear_probe_corrs)[::-1]
			...
			plt.axis('off')
			plt.savefig(os.path.join(work_dir,pert_desc + '-heatmap.png'))
