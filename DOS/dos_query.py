...
qCurs = signature.find({'pert_type':'trt_cp','cell_id':cell1,'is_gold':1.0},{'cell_id':True,'pert_type':True,'sig_id':True,'pert_desc':True,'is_gold':True,'pert_dose':True,'pert_time':True,'cell_id':True,'pert_type':True})
qLis = []
for x in qCurs: 
    qLis.append(x)
    
prtTypes = [str(x['pert_type']) for x in qLis]
uPrtTypes = set(prtTypes) #list unique pert types
