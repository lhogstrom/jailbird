...
############################################################################################################
  ...
  # Pre-processing: rank normalization for CC datasets
   ...
   # path1 <- '/xchip/cogs/projects/NMF/TA_lung_cancer_genomic_perturbations/shRNA_pilot/NMF_projection_c20_viz'
   # prefix1 <- 'TA.KD009_KD010_A549_96H_mod_sig_id'
   path1 <- '/xchip/cogs/projects/NMF/TA_lung_cancer_genomic_perturbations/oe_pilot/NMF_projection_c9_viz'
   prefix1 <- 'TA.OE006_A549_96H_mod_sig_id'   
   ...
   ##  CMAP Compound Classes ----------------------------------------------------------------------------------------------------------  
      ...
      # annot.file   <- paste(path1,"/TA.KD009_KD010_A549_96H_annotations.txt",sep="")
      annot.file   <- paste(path1,"/TA.OE006_A549_96H_mod_sig_id_annotations.txt",sep="")
      # Parameters
      k.comp       <- 9    # Optimal number of components: 9, 20
      name.column  <- 2     # Column # in annot.file containing the perturbation name
