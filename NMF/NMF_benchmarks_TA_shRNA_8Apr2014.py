...
def MI_pairwise_comp(MI_matrix,cliqFrm,anntFrm,graphDir):
    ...
    for r in cliqFrm.iterrows():
        grp = r[1]['id']
        brds = r[1]['sig']
        anntMtch = anntFrm[anntFrm.index.isin(brds)]
        anntNonMtch = anntFrm[~anntFrm.index.isin(brds)]
...
def mi_heatmap_group(mtrx,graphDir,grpSort,space_name='input_space'):
    ...
    plt.ylabel('CMAP signatures')
    plt.title('mutual information - ' + space_name)
    # smFrm = pd.DataFrame(grpSort.pert_id)
...
def group_component_maps(Hmtrx,cliqFrm,graphDir):
    ...
    '''
    # make new dir for graphs 
    graphDir2 = graphDir + '/group_component_maps'
    if not os.path.exists(graphDir2):
        os.mkdir(graphDir2)
    ...
    for r in cliqFrm.iterrows():
        ...
        outF = os.path.join(graphDir2,grpMod+'.png')
        plt.savefig(outF, bbox_inches='tight')
        plt.close()
